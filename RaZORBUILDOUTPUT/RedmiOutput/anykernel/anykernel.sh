...
chmod 755 /system/etc/mixer_paths.xml
...
found=$(find init.rc -type f | xargs grep -oh "import /init.razor.rc");
if [ "$found" != 'import /init.razor.rc' ]; then
	#append the new lines for this option at the bottom
        echo "" >> init.rc
	echo "import /init.razor.rc" >> init.rc
fi
